...
namespace Game.Gameplay.EventBus.Handlers.Effects
{
    ...
    public sealed class HealEffectHandler : IInitializable, IDisposable
    {
        ...
        {
            var componentLife = evt.Target.Get<Component_Life>();
            var amount = (int)(evt.BaseHealingAmount + evt.MaxHealthMultiplication * componentLife.maxHealth.Value);
            
            componentLife.health.Value = Mathf.Min(componentLife.maxHealth.Value, componentLife.health.Value + amount);
        }
    }
}